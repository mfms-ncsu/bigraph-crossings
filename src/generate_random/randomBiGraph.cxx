...
<< endl;
  ...
  info_stream << "generated by randomBiGraph in bigraph-crossings repository "
      << c_time_string << endl;
  int n0;
  ...
  info_stream << " connectedness = " << connectedness;
  if ( m > n0 * n1 ) {
  ...
  info_stream << " dimension = " << dimension;
  ...
  for ( int subject = 1; subject <= number_of_subjects; ++subject ) {
    ...
    seed_stream << initial_random_stream;
    ostringstream info_with_seed;
    info_with_seed << info_stream.str() << " seed = " << seed_stream.str();
    print_dot_output( dot_output, base_name.str().c_str(),
                      info_with_seed.str().c_str(), G, n0 );
    print_ord_output( ord_output, base_name.str().c_str(),
                      seed_stream.str().c_str(), G, n0 );
  }
